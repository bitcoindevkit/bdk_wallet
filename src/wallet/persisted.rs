...
// ///
// /// Associated functions of this trait should not be called directly, and the trait is designed
// so /// that associated functions are hard to find (since they are not methods!).
// [`WalletPersister`] is /// used by [`PersistedWallet`] (a light wrapper around [`Wallet`]) which
// enforces some level of /// safety. Refer to [`PersistedWallet`] for more about the safety checks.
...
//     ///
//     /// The implementation must return all data currently stored in the `persister`. If there is
// no     /// data, return an empty changeset (using [`ChangeSet::default()`]).
//     ///
...
//     /// Initialize the `persister` and load all data.
//     ///
//     /// This is called by [`PersistedWallet::create_async`] and [`PersistedWallet::load_async`]
// to     /// ensure the [`AsyncWalletPersister`] is initialized and returns all data in the
// `persister`.     ///
//     /// # Implementation Details
//     ///
//     /// The database schema of the `persister` (if any), should be initialized and migrated here.
...
// ///
// /// This is a light wrapper around [`Wallet`] that enforces some level of safety-checking when
// used /// with a [`WalletPersister`] or [`AsyncWalletPersister`] implementation. Safety checks
// assume that /// [`WalletPersister`] and/or [`AsyncWalletPersister`] are implemented correctly.
// ///
// /// Checks include:
// ///
...
// /// * Ensure the wallet is persisted to the same `P` type as when created/loaded. Note that this
// is ///   not completely fool-proof as you can have multiple instances of the same `P` type that
